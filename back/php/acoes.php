...
if (isset($_POST['acao']) && $_POST['acao'] === 'adicionar_favorito') {
    ...
    if (mysqli_num_rows($check) > 0) {
        echo json_encode(['success' => false, 'message' => 'Essa espécie já está nos seus favoritos!']);
        exit;
    }
    ...
    if (mysqli_query($conn, $sql)) {
        echo json_encode(['success' => true, 'message' => 'Espécie adicionada aos favoritos!']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Erro ao favoritar: ' . mysqli_error($conn)]);
    }
    exit;
}

// --- REMOVER FAVORITO (AJAX) ---
if (isset($_POST['acao']) && $_POST['acao'] === 'remover_favorito') {
    ...
    $sql = "DELETE FROM favorito WHERE id_usuario = '$id_usuario' AND id_especie = '$id_especie'";
...
if (isset($_POST['upload_artigo'])) {
    ...
    // Verifica se o PDF foi enviado corretamente
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $destino_pdf)) {
            // Insere no banco com capa (se houver)
